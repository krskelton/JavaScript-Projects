...
const Calculator = {
    //This value displays 0 on the screen
    Display_Value: '0',
    ...
    //this will hold the operator, we sset it to null for now
    operator: null,
};
...
function Handle_Operator(Next_Operator){
    ...
    //when an opertor key is pressed, we convert the current number displayed on the screen to a number and then store the result in Calculator.First_Operand if it doesn't already exist.
    const Value_of_Input = parseFloat(Display_Value);
    //checks if an operator already exists and if Wait_Second_operand is true, then updates the operator and exists from the function
    ...
    if(First_Operand == null){
        ...
        //if opreator exists, property lookup is performed for the operator in the Perform_Calculation object and the function that matches the operator is executed
